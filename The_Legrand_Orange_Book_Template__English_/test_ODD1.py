...
print(f"ODD1: score={odd_global:.4f}, {len(odd_indicators)} indicateurs")
...
colors = ['#87CEEB', '#4A90E2', '#0055B8', '#003D82']  # Light to dark blue
...
# Threshold labels
thresholds = [0.0, 0.3, 0.6, 0.8, 1.0]
...
for threshold, angle in zip(thresholds, threshold_angles):
    ...
    ax1.text(x, y, f'{threshold:.1f}', ha='center', va='center', 
...
# Horizontal bars (reversed order: lowest scores first)
...
# Add legend - removed
# legend_elements = [
#     Patch(facecolor='#87CEEB', label='Seuil min (0.50)'),
#     Patch(facecolor='#0055B8', label='Cible excellente (0.80)')
# ]
# ax2.legend(handles=legend_elements, loc='lower right', fontsize=9)

plt.tight_layout()
os.makedirs('Pictures/graphs', exist_ok=True)
output_path = 'Pictures/graphs/ODD1_scores.png'
plt.tight_layout()
plt.savefig(output_path, dpi=300, bbox_inches='tight', facecolor='white')
plt.close(fig)

print(f"[OK] ODD1 saved to {output_path}")
